...
connection.connect((err) => {
 ...
 router.get('/', function(req, res) {
  ...
  router.post('/', function(req, res) {
    ...
    connection.query('SELECT user.*, role.role_name, role.max_borrow FROM user INNER JOIN role ON user.role_id = role.role_id WHERE library_id = ? AND role_name = "admin" AND is_deleted = 0', [library_id], function(error, results, fields) {
